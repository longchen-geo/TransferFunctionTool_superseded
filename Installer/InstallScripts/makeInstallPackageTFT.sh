...
LONG=`grep BuildConfiguration $APP.pro.user | grep Release | grep -v shadowDir`
START=${LONG#*">"}
build_folder=${START%"<"*}
...
then
	...
	#
	    ...
	    echo
	    echo "*** Build Release version of Transfer Function Tool before running this script. ***"
	    echo
	    exit 1
